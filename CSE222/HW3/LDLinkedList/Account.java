...
public class Account {
    ...
    public Account(int accountId, String username, String birthdate, String location){
        ...
        this.inboxMessages = new LDLinkedList<Message>();
        this.outboxMessages = new LDLinkedList<Message>();
     ...
     */
    public void login(LDLinkedList<Account> currentAccounts){
        ...
        if(this.isLogin()){
            ...
            if(postExist){
                if(user.posts.get(index).uncommentPosts(this)){
                    this.addHistory(new String("You have uncommented on " + user.getUsername() + "'s post id: " + postId));
                }
            }
            else{
                System.err.println("There is no such post with " + postId + " in the account " + user.getUsername());
     ...
     * @param currentAccounts The Accounts that has been created so far.
     */
    public void viewInteractions(Account user, LDLinkedList<Account> currentAccounts){
        if(this.isLogin()){
            if(user.posts != null){
                for(int i=0; i<user.posts.size(); ++i){
    ...
    public void unfollow(Account user){
        if(this.isLogin()){
            ...
            if(isFollowing){
                ...
                System.err.println(this.getUsername() + " cannot unfollow " + user.getUsername() + " because " + this.getUsername() + " is not following " + user.getUsername());
                return;
            }
        }
    }
     ...
     * 
     * @param user The Account user that will be unblocked.
     */
    ...
    public void sendMessage(Account user, Message message){
        if(this.isLogin()){
            ...
            for(int i=0; i<user.blockedAccounts.size(); ++i){
                if(!user.blockedAccounts.lazyDeleted(i)){
                    if(user.blockedAccounts.get(i).getUsername().equals(this.getUsername())){
                        isBlocked = true;
                        break;
                    }
                }
            ...
            for(int i=0; i<this.following.size(); ++i){
                if(!this.following.lazyDeleted(i)){
     ...
     * @param currentAccounts The Accounts that has been created so far.
     */
    public void viewOutbox(LDLinkedList<Account> currentAccounts){
        if(this.isLogin()){
   ...
   /**
     ...
     * @param currentAccounts The Accounts that has been created so far.
     */
    public void viewInbox(LDLinkedList<Account> currentAccounts){
        if(this.isLogin()){
    ...
    public void viewProfile(Account user){
        if(this.isLogin()){
            ...
            System.out.println(user.getUsername() + " is following " + (user.following.size() - user.following.getLazyDeleteCount())  + " account(s) and has " + user.followers.size() + " followers.");
            if(user.followers.size() != 0){
                System.out.print("The followers of " + user.getUsername() + " are: ");
                for(int i=0; i<user.followers.size(); ++i){
                    if(!user.followers.lazyDeleted(i)){
            ...
            if(user.following.size() != 0){
                ...
                for(int i=0; i<user.following.size(); ++i){
                    if(!user.following.lazyDeleted(i)){
