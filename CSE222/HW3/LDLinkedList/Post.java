...
public class Post {
    ...
    public boolean unlikePosts(Account user){
        ...
        if(isLiked){
            ...
            System.err.println(user.getUsername() + " did not like post with Id " + this.getPostId() + " before, so he/she cannot unlike it.");
            return false;
        }
    }
     ...
     */
    public void interactions(LDLinkedList<Account> currentAccounts){
        System.out.println("(Post Id: " + postId + ") " + content);
        if(this.likes.size() - this.likes.getLazyDeleteCount() !=  0){
            System.out.print("This post was liked by the following account(s): ");
            for(int i=0; i<likes.size(); ++i){
                if(!likes.lazyDeleted(i)){
        ...
        if(this.comments.size() - this.comments.getLazyDeleteCount() != 0){
            System.out.println("This post has " + (this.comments.size() - this.comments.getLazyDeleteCount()) + " comment(s).");     
            for (int i = 0; i < comments.size(); ++i) {
                if (!comments.lazyDeleted(i)) {
